  ...
  return (
    <div>
        <div className='w-full h-full relative'>
          ...
          {PostLists.map((post)=>(
        <div className='w-full h-auto mx-auto py-6 md:py-12 container bg-white grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 items-start justify-center gap-5 px-3'>
              {IsAuth && post.author.id === (auth.currentUser && auth.currentUser.uid) && (
                ...
                <nav className='flex p-2 md:p-3 rounded-b-md text-gray-700 font-semibold items-center justify-between'>
                  ...
                  <div className='text-red-700 text-xs md:text-sm capitalize active:text-red-400 hover:text-red-400'>
                  {IsAuth && post.author.id === (auth.currentUser && auth.currentUser.uid) && (
                    <button onClick={()=>{deletPost(post.id)}}> Delete my post
                    {/* <FaBitbucket/> */}
                    </button>
                    )}
                  </div>
